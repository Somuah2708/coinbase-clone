/**
 * SignIn Page - User login form with email and password fields.
 * Styled to match Coinbase's clean authentication design.
 */
...
const SignIn = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Mock sign in — no real auth needed for the assignment
      ...
      <div className="w-full max-w-[520px] mx-auto px-6">
        ...
        <div className="bg-white rounded-3xl shadow-xl p-10 sm:p-14">
          ...
          <p className="text-[16px] text-[#5b616e] text-center mb-10">
            Not a Coinbase user?{" "}
            <Link to="/signup" className="text-[#0052FF] font-semibold hover:underline">
              Sign up
            </Link>
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-5 h-5 rounded border-[#d1d5db] accent-[#0052FF]" />
                <span className="text-[14px] text-[#5b616e]">Remember me</span>
              </label>
              <a href="#" className="text-[14px] text-[#0052FF] font-semibold hover:underline">
                Forgot password?
              </a>
            </div>
